...
import {banks, types, focuses, focusNames, resolve, choose, marks, historicalSet, contentChanged, ensureBank, loadSet, challengeLevels, challengeLabel, availableChallenges, puzzlePool, examSubtopics, nextExamSubtopic, matchesSubtopic, hasAlternativeExamSet} from './bank.js';
...
async function start(set,{force=false,challengeLevel=null,inSequence=false}={}){
  ...
  data.active={id:crypto.randomUUID(),contentVersion:BANK_VERSION,code:set.code,mathsTopic:set.mathsTopic??'all',examSubtopic:set.type===1?(set.examSubtopic??'all'):'all',inSequence:set.type===1&&inSequence,answers:{},checks:{},first:{},hints:{},reveals:{},started:now,origin:now,saved:now,deadline:set.minutes?now+set.minutes*60000:null,finished:null,outcome:null,timingEvents:[],tracking:attemptEligibility(data,set.code,now)};
...
function submit(outcome){
  ...
  const record={id:a.id,code:encode({...set,version:BANK_VERSION}),type:set.type,focus:set.focus,finished:now,earned,max:set.total,percentage:Math.round(earned/set.total*100),seconds:Math.max(0,Math.round((stop-a.origin)/1000)),totalSeconds:Math.max(0,Math.round((stop-a.started)/1000)),attemptChecks:Object.values(a.checks).reduce((s,n)=>s+n,0),assisted:Object.keys(a.hints).length>0||Object.keys(a.reveals).length>0,outcome,firstEarned,firstMax,partScores:partScores(set,a.first,finalResults)};
...
function progress(){
  ...
  $('#backup-file').onchange=async e=>{
    try{
      ...
      if(imported.recentAttempts!==undefined){
