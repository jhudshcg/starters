import {interactiveKinds} from './puzzle-rules.js';
import {renderPuzzle,bindPuzzles} from './puzzle-controls.js';
...
try{data=loadStorage();}catch{storageError();}
...
function questionHTML(q,index,set){
  ...
  return `<article class="question type-${set.type}" data-question="${q.slot}"><div class="question-top"><span class="question-number">${String(index+1).padStart(2,'0')}</span><div><h2>${esc(q.title)}</h2><div class="muted">${esc(q.format)} · <span title="Individual question code">${questionCode(set.type,q.slot,q.variation)}</span>${q.tags.filter(t=>t.startsWith('CA')).length?` · ${q.tags.filter(t=>t.startsWith('CA')).join(', ')}`:''}</div></div><span class="marks-badge">${marks(q)} ${set.type===0?'points':'marks'}</span></div><p class="question-prompt">${esc(q.prompt)}</p><div class="question-layout ${q.code||q.grid||q.clues?'':'no-stimulus'}">${stimulus(q)}<div class="parts">${q.parts.map((p,j)=>{
...
function activity(){
  ...
  main.innerHTML=`<a href="#home" class="back">← All activities</a><div class="page-top"><div><div class="eyebrow">${types[set.type].name} / ${set.questions.length===1?'Single question':'Starter set'}</div><h1>${esc(focusNames[set.focus])}</h1><p class="muted">${set.questions.length} ${set.questions.length===1?'question':'questions'} · ${set.total} ${set.type===0?'points':'marks'} · About ${types[set.type].minutes} minutes</p></div><div class="focus-row"><label for="focus">Focus</label><select id="focus">${focuses(set.type).map(f=>`<option value="${f}" ${f===set.focus?'selected':''}>${esc(focusNames[f])}</option>`).join('')}</select></div></div>
  ...
  <div class="random-controls"><button id="new-type">Get new question set ↗</button><button id="new-focus" ${!sameFocusAlternatives?'disabled title="All templates in this focus are already shown. Use Get new permutation."':''}>New set in this focus</button><button id="permutation">Get new permutation ↻</button></div>
  ...
  bindPuzzles(main,set,a,(slot,id,value,selector)=>{
    ...
    const control=main.querySelector(selector);
