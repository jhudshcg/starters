...
Question slots are stable assigned IDs, not positions in a sorted array. A bank version fixes question content, variation ordering, answers and generation rules. Retain old versioned banks; do not reuse a version for changed content or wrap version numbers on exhaustion. Compatible additions may use previously unassigned slots without altering existing mappings. Answer-changing corrections require a new bank version. App styling changes do not consume a bank version.
