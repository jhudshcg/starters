...
Keep the slot and variation positions for corrections and refinements. A substantive change to the task, assessed concept or required solution is a new question with a new, unused slot. Never reorder variations or recycle removed slots/variation positions. Prefer `retired: true` for superseded questions: they leave random selection but remain accessible through shared codes. Remove a question only when it should no longer be accessible. Fingerprint updates produce a discreet notice on old codes, not invalidation; keep running `codes:update` to preserve historical metadata.
